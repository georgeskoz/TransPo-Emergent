          ...
          <Card className="bg-noir-600 border-noir-300">
            ...
            <CardContent className="space-y-6">
              <p className="text-sm text-noir-100">
                For Quebec drivers: If you are registered for GST/QST, provide your tax numbers below for proper invoicing.
              </p>

              <div className="grid md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <Label className="text-noir-100">GST Number (TPS)</Label>
                  <Input
                    value={profile.gst_number}
                    onChange={(e) => setProfile({ ...profile, gst_number: e.target.value })}
                    placeholder="123456789RT0001"
                    className="bg-noir-500 border-noir-300 text-white font-mono"
                    data-testid="gst-number-input"
                  />
                  <p className="text-xs text-noir-100">15-character GST/HST account number</p>
                </div>
                <div className="space-y-2">
                  <Label className="text-noir-100">QST Number (TVQ)</Label>
                  <Input
                    value={profile.qst_number}
                    onChange={(e) => setProfile({ ...profile, qst_number: e.target.value })}
                    placeholder="1234567890TQ0001"
                    className="bg-noir-500 border-noir-300 text-white font-mono"
                    data-testid="qst-number-input"
                  />
                  <p className="text-xs text-noir-100">16-character QST account number</p>
                </div>
              </div>

              <div className="grid md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <Label className="text-noir-100">Mandatory Billing Number</Label>
                  <Input
                    value={profile.billing_number}
                    onChange={(e) => setProfile({ ...profile, billing_number: e.target.value })}
                    placeholder="Enter billing number"
                    className="bg-noir-500 border-noir-300 text-white font-mono"
                    data-testid="billing-number-input"
                  />
                </div>
                <div className="space-y-2">
                  <Label className="text-noir-100">SRS Authorization Code</Label>
                  <Input
                    value={profile.srs_code}
                    onChange={(e) => setProfile({ ...profile, srs_code: e.target.value })}
                    placeholder="Enter SRS code"
                    className="bg-noir-500 border-noir-300 text-white font-mono"
                    disabled={!profile.srs_available}
                    data-testid="srs-code-input"
                  />
                </div>
              </div>

              {/* Checkboxes */}
              <div className="space-y-4 pt-4 border-t border-noir-300">
                <label className="flex items-start gap-3 cursor-pointer group">
                  <div className="relative mt-0.5">
                    <input
                      type="checkbox"
                      checked={profile.srs_available}
                      onChange={(e) => setProfile({ ...profile, srs_available: e.target.checked, srs_code: e.target.checked ? profile.srs_code : '' })}
                      className="sr-only peer"
                      data-testid="srs-available-checkbox"
                    />
                    <div className={`w-5 h-5 rounded border-2 flex items-center justify-center transition-all ${
                      profile.srs_available 
                        ? 'bg-cyan border-cyan' 
                        : 'border-noir-300 group-hover:border-noir-200'
                    }`}>
                      {profile.srs_available && <Check className="w-3 h-3 text-black" />}
                    </div>
                  </div>
                  <span className="text-sm text-noir-100">
                    I have an SRS authorization code
                  </span>
                </label>
                ...
                <label className="flex items-start gap-3 cursor-pointer group">
                  <div className="relative mt-0.5">
                    <input
                      type="checkbox"
                      checked={profile.tax_disclaimer_accepted}
                      onChange={(e) => setProfile({ ...profile, tax_disclaimer_accepted: e.target.checked })}
                      className="sr-only peer"
                      data-testid="tax-disclaimer-checkbox"
                    />
                    <div className={`w-5 h-5 rounded border-2 flex items-center justify-center transition-all ${
                      profile.tax_disclaimer_accepted 
                        ? 'bg-cyan border-cyan' 
                        : 'border-noir-300 group-hover:border-noir-200'
                    }`}>
                      {profile.tax_disclaimer_accepted && <Check className="w-3 h-3 text-black" />}
                    </div>
                  </div>
                  <span className="text-sm text-noir-100">
                    I confirm that I am registered for GST/QST collection, and that Transpo is not responsible for my tax obligations. I understand that I am solely responsible for collecting and remitting applicable taxes to the relevant tax authorities.
                  </span>
                </label>
              </div>
              ...
              <div className="p-4 rounded-lg bg-cyan/10 border border-cyan/30">
                <h4 className="text-cyan font-medium mb-2 flex items-center gap-2">
                  <AlertCircle className="w-4 h-4" />
                  Important Tax Information
                </h4>
                <ul className="text-sm text-noir-100 space-y-1 list-disc list-inside">
